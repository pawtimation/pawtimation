...
export function OwnerLogsContent() {
  ...
  async function loadLogs() {
    ...
    try {
      ...
      // Update offset from server response but preserve local limit setting
      if (data.pagination) {
        setPagination(prev => ({
          limit: prev.limit,
                  ...
                  <tr key={log.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-sm">
                      <details className="cursor-pointer">
                        <summary className="text-teal-600 hover:text-teal-700 text-xs font-medium">
                          View Details
                        </summary>
