...
export function StaffMobileJobDetail() {
  ...
  async function handleMediaDelete(mediaId) {
    ...
    try {
      const response = await staffApi(`/media/${mediaId}`, {
        method: 'DELETE'
      });
      
      if (response.ok) {
        setWalkMedia(prev => prev.filter(m => m.id !== mediaId));
      } else {
        const error = await response.json();
              ...
              <div key={media.id} className="relative aspect-square rounded-lg overflow-hidden bg-slate-100">
                ...
                {!isCompleted && !isCancelled && (
                  <button
                    ...
                    className="absolute top-1 right-1 w-7 h-7 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center shadow-lg transition-colors disabled:opacity-50"
                    title="Delete photo"
                  >
