    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="px-10 py-6 space-y-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-1">Overview of your bookings, clients and revenue.</p>
          <p className="text-sm text-teal-600 mt-2">{getGreeting()}, {user?.name?.split(' ')[0] || 'there'} — here's what's happening today.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div 
            ...
            className="relative rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all text-white overflow-hidden cursor-pointer" 
            style={{background: 'linear-gradient(to bottom right, #3F9C9B, #66B2B2, #A8E6CF)'}}
          >
            <div className="absolute top-0 right-0 w-32 h-32 bg-white opacity-10 rounded-full -mr-16 -mt-16"></div>
            <div className="relative">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium text-white/90">Today's Jobs</p>
              ...
              <p className="text-5xl font-bold mb-1">{stats.todayJobs.total}</p>
              <p className="text-xs text-white/80">{stats.todayJobs.completed} completed · {stats.todayJobs.upcoming} upcoming</p>
            </div>
          </div>
          ...
          <div className="relative bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-gray-600">This Week's Jobs</p>
            ...
            <p className="text-5xl font-bold text-gray-900 mb-1">{stats.weekJobs.total}</p>
            <p className="text-xs" style={{color: stats.weekJobs.change >= 0 ? COLORS.success : COLORS.error}}>
              {stats.weekJobs.change >= 0 ? '+' : ''}{stats.weekJobs.change} vs last week
            </p>
          </div>
          ...
          <div className="relative bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all">
            ...
            <p className="text-xs text-gray-500">{stats.newClientsThisMonth} new this month</p>
          </div>
          ...
          <div className="relative bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-2">
              ...
              <svg className="w-10 h-10 opacity-40" style={{color: COLORS.tealLight}} fill="currentColor" viewBox="0 0 20 20">
                <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z"/>
              </svg>
            </div>
            <p className="text-5xl font-bold text-gray-900 mb-1">{stats.activeStaff.active}</p>
            <p className="text-xs text-gray-500">{stats.activeStaff.total} total staff</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div 
            ...
            className="relative bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between mb-2">
            ...
            <p className="text-5xl font-bold text-gray-900 mb-1">{formatCurrency(stats.revenue7Days.totalCents)}</p>
            <p className="text-xs" style={{color: stats.revenue7Days.change >= 0 ? COLORS.success : COLORS.error}}>
              {stats.revenue7Days.change >= 0 ? '+' : ''}{formatCurrency(stats.revenue7Days.change)} vs previous 7 days
            </p>
          </div>

          {/* Unpaid Invoices */}
          <div className="relative rounded-2xl p-6 shadow-sm hover:shadow-md transition-all text-white" style={{background: `linear-gradient(to bottom right, ${COLORS.amber}, #FCD34D)`}}>
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-white/90">Unpaid Invoices</p>
              <svg className="w-10 h-10 text-white/30" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd"/>
              </svg>
            </div>
            <p className="text-5xl font-bold mb-1">{formatCurrency(stats.unpaidInvoices.totalCents)}</p>
            <p className="text-xs text-white/80">{stats.unpaidInvoices.count} invoice{stats.unpaidInvoices.count !== 1 ? 's' : ''}</p>
          </div>
          ...
          <div className="relative rounded-2xl p-6 shadow-sm hover:shadow-md transition-all text-white" style={{
            ...
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-white/90">Overdue Invoices</p>
            ...
            <p className="text-5xl font-bold mb-1">{formatCurrency(stats.overdueInvoices.totalCents)}</p>
            {stats.overdueInvoices.count > 0 ? (
              <p className="text-xs text-white/80">{stats.overdueInvoices.count} invoice{stats.overdueInvoices.count !== 1 ? 's' : ''}</p>
            ) : (
          ...
          <div className="relative bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all">
            ...
            <p className="text-5xl font-bold text-gray-900 mb-1">{formatCurrency(stats.paidThisMonth.totalCents)}</p>
            <p className="text-xs text-gray-500">{stats.paidThisMonth.count} invoice{stats.paidThisMonth.count !== 1 ? 's' : ''}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
            <div>
              <h3 className="text-xl font-bold text-gray-800">Service breakdown</h3>
              <p className="text-sm text-gray-500 mt-1">Share of bookings by service type</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
            <div className="mb-6">
              <h3 className="text-xl font-bold text-gray-800">Action centre</h3>
              <p className="text-sm text-gray-500 mt-1">Things that need your attention</p>
            </div>
          ...
          <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-500 mt-1">Latest changes across your business</p>
            </div>
